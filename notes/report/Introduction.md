...
From the above image there is 2 main resolution features, nano spheres or nano grids. Both serve different purposes, grids are typically used for checking magnification, distortion and calibration of tip shape, sensitivity, etc. While tin spheres with their varied spacing is used for exposure and light testing. Given that this project is more focused on lithography fabrication methods , it will aim to focus on the grid resolution.
...
### 1.3 Fabrication review

#### Fabrication approaches for near-vertical sidewall standards

Given the importance of sidewall angle, several approaches have been explored to improve its control in the fabrication of resolution standards.

The dominant approach has been single-crystal silicon etching, pioneered at NIST. By exploiting the crystallographic planes of silicon, wet anisotropic etching with KOH produces sidewalls whose angle is determined by the crystal lattice rather than by the lithographic process, giving intrinsically well-defined and reproducible geometry. NIST's single-crystal critical dimension reference material (SCCDRM), calibrated traceably via CD-AFM and laser interferometry, represents the current primary national standard for sidewall angle calibration [9].

A second route, pursued by multiple groups primarily for photonic device fabrication, combines EBL with inductively coupled plasma (ICP) etching and optimised mask materials. Using HSQ as the etch mask rather than chromium or organic resists, sidewall roughness below 10 nm in silicon has been achieved, two to three times better than earlier processes, by suppressing mask-edge roughness transfer during the etch step [10]. Cryogenic deep reactive ion etching (DRIE) has extended this further, with O₂ flow optimisation used to suppress lateral etching and produce nearly vertical profiles in silicon pillar arrays [11].

####  Shortcomings of existing approaches

Despite these advances, each approach carries limitations that motivate the present work.

Single-crystal silicon etching produces sidewall angles that are crystallographically fixed, typically 54.7° for (100) silicon etched in KOH, and cannot be tuned to 90°. Achieving vertical sidewalls requires either a different crystal orientation or additional processing steps such as thermal oxidation smoothing, adding complexity and reducing throughput [9].

EBL-based approaches suffer from the fundamental proximity effect: backscattered electrons expose resist beyond the intended boundary, and this effect worsens with feature depth. Achieving consistent sidewall verticality across a full grid geometry, where adjacent features influence one another's dose, is therefore difficult to guarantee, particularly at depths beyond a few hundred nanometres [12]. The proximity effect of EBL will be further discussed later. The ICP etching step introduces its own variability through plasma non-uniformity and mask erosion, and the multi-step process (EBL → etch → ALD smoothing) requires careful process control at each stage to prevent degradation of the sidewall angle.
...
<ol>
  ...
  <li>C. Gozzi et al., "Silicon metalens fabrication from electron beam to UV-nanoimprint lithography," <em>Nanomaterials</em>, vol. 11, no. 9, 2021. DOI: <a href="https://doi.org/10.3390/nano11092329">10.3390/nano11092329</a></li>
